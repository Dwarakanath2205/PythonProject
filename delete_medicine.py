...
def delete():
    ...
    # Delete the record with the entered MedicineName
    cursor.execute("DELETE FROM 'medicine' WHERE MedicineName = ?", (entered_name,))
    conn.commit()
    conn.close()

    # Clear the Treeview to refresh the display
    for item in tv.get_children():
        tv.delete(item)

    # Show success or info message
    msg.showinfo("Success",'MedicineName deleted successfully')

# Corrected button reference and command
